...
MuJoCo 是第一个进程内适配器。它接受包中立的 `SceneCfg`，在构造时物化 XML，并通过 `unisim.SimBackend` 暴露缓存数值状态；任务拥有的场景组装仍留在 UniLab。它的原生批处理执行器是 mjbatch（`unilabsim/mjbatch`，`kevinzakka/mjbatch` 的维护 fork）；不支持异构模型变体，字段级域随机化通过 mjbatch `expand` 与 `set_const` 完成。
